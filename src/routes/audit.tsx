  ...
  return (
    <div className="px-3 py-4 space-y-4 sm:px-4">
      <div className="border-b border-border/60 pb-3">
        <h1 className="text-base font-semibold tracking-tight sm:text-lg">Audit log</h1>
        <p className="text-xs text-muted-foreground">Role changes and critical actions</p>
      </div>
...
function DiffView({ row }: { row: AuditRow }) {
  ...
  if (row.action === "update" && diff && "old" in diff && "new" in diff) {
    ...
    return (
      <div className="max-h-80 overflow-auto rounded-xl border border-border/70 shadow-sm">
        <table className="w-full text-xs">
          ...
          <tbody>
            {changed.map((k) => (
              <tr key={k} className="border-t border-border/60 transition-colors hover:bg-muted/30">
  ...
  return (
    <div className="max-h-80 space-y-1 overflow-auto rounded-xl border border-border/70 shadow-sm p-2 text-xs">
      {entries.length === 0 && (
