...
export const Route = createFileRoute("/bulk-lab")({
  head: () => ({ meta: [{ title: "Bulk Lab — Slitting Planner" }] }),
  ...
  return (
    <div className="px-3 py-3 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-sm font-semibold tracking-tight">Bulk combination lab</h1>
      ...
      {lastSummary && (
        <div className="grid grid-cols-2 gap-2 rounded-md border bg-card p-2 text-center text-xs sm:grid-cols-4">
          <div>
      ...
      <div>
        <h2 className="mb-2 text-xs font-semibold text-muted-foreground">
          Approval queue ({queue.data?.length ?? 0})
        </h2>
        {queue.isLoading ? (
...
function ExhaustiveDiscoveryPanel() {
  ...
  const annotatedResults = useMemo(() => {
    ...
    if (targetSet.size > 0) {
      annotated.sort(
        (a, b) =>
          b.targetSlitCount - a.targetSlitCount ||
          b.distinctTargets - a.distinctTargets ||
          a.row.combo.scrap_mm - b.row.combo.scrap_mm ||
          a.row.combo.knife_count - b.row.combo.knife_count,
      );
    }
    return annotated;
  ...
  const visibleResults = useMemo(() => {
    if (!annotatedResults) return null;
    return hideExisting ? annotatedResults.filter((r) => !r.existsOnMachine) : annotatedResults;
  }, [annotatedResults, hideExisting]);

  const existingHiddenCount = annotatedResults
    ? annotatedResults.length - (visibleResults?.length ?? 0)
    : 0;
  ...
  const approveOne = async (
    ...
    try {
      ...
      if (!ok) {
        toast.error(
          "Couldn't save — one or more slit widths have no linked product yet. Map a product in the Slits page and retry.",
        );
        return;
      }
      toast.success(
    ...
    <div className="rounded-md border bg-card p-3 space-y-3">
      <div>
        <h2 className="flex items-center gap-1.5 text-xs font-semibold">
          <FlaskConical className="h-3.5 w-3.5" /> Exhaustive combination discovery
        </h2>
        <p className="text-[11px] text-muted-foreground">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <div className="space-y-1">
          ...
          <Select value={targetMachine} onValueChange={setTargetMachine}>
            ...
            <SelectContent>
              <SelectItem value="null">Unassigned (draft)</SelectItem>
              {machines.data?.map((m) => (
                <SelectItem key={m.machine_id} value={m.machine_id.toString()}>
                  {m.code}
                </SelectItem>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <label className="ml-auto flex items-center gap-1.5 text-[11px]">
          <Switch checked={hideExisting} onCheckedChange={setHideExisting} />
          <span className="text-muted-foreground">Hide already on {machineLabel}</span>
        </label>
      </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-medium">
              {visibleResults?.length ?? 0} shown · {annotatedResults.length} found
              {truncated ? " (truncated — raise max results)" : ""}
              {existingHiddenCount > 0 && (
                <span className="text-muted-foreground"> · {existingHiddenCount} already on {machineLabel}</span>
              )}
            </p>
            <p className="text-[10px] text-muted-foreground">
              {isMultiCoil
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="font-mono">
                {(visibleResults ?? []).map((r, i) => {
                  ...
                  return (
                    <tr key={`${r.row.coilSpecId ?? "no"}:${r.canonicalKey}`} className="border-t align-top">
                      ...
                      <td className="px-2 py-1">
                        <div className="flex flex-wrap items-center gap-1">
                          {!r.row.coilSpecId ? (
                            ...
                            <Badge variant="secondary" className="px-1.5 py-0 text-[10px]">On {machineLabel}</Badge>
                          ) : r.approved ? (
                            <Badge variant="outline" className="px-1.5 py-0 text-[10px]">Other machine</Badge>
                          ) : r.exists ? (
                            <Badge variant="outline" className="px-1.5 py-0 text-[10px]">Draft</Badge>
                          ) : (
                          ...
                          {targetSet.size > 0 && (
                            <Badge
                              variant="outline"
                              className="border-primary/40 bg-primary/5 px-1.5 py-0 text-[10px] font-mono text-primary"
                              title={`${r.distinctTargets} distinct target width(s) · ${r.targetSlitCount} target slit(s)`}
                            >
                              {r.distinctTargets}/{targetSet.size} tgt · {r.targetSlitCount}×
                            </Badge>
                          )}
                        </div>
                      </td>
                      <td className="px-2 py-1 text-right">
                        {canApprove ? (
                          <Button
                            ...
                            title={
                              !r.row.coilSpecId
                                ? "Add this coil_spec in the Coils page first"
                                : r.existsOnMachine
                                  ? `Already on ${machineLabel} — duplicates are blocked`
                                  : r.missingProduct && !isDraftSave
                ...
                {visibleResults && visibleResults.length === 0 && (
                  <tr>
                    <td colSpan={isMultiCoil ? 11 : 10} className="px-2 py-3 text-center text-[11px] text-muted-foreground">
                      Every candidate is already on {machineLabel}. Switch target machine or toggle "Hide already on {machineLabel}" off.
                    </td>
                  </tr>
                )}
              </tbody>
