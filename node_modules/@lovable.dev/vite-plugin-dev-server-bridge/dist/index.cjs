...
var PREVIEW_VIEWERS_PATH = "/_sandbox/preview/viewers";
...
function devServerBridgePlugin() {
    ...
    return [
      {
        tag: "script",
        attrs: { type: "module" },
        // Static import: Rolldown's lazy compilation (dynamic imports) emits a raw
        // browser import of the \0-prefixed virtual id, which can never resolve.
  ...
  return {
    ...
    configResolved(config) {
      ...
      isBundledDev = experimental?.bundledDev === true;
    },
    configureServer(srv) {
...
`, map: null };
    ...
    // Both hook shapes: Vite 3 resolves only the legacy {enforce, transform} keys and
    // otherwise registers `undefined` as a hook, 500ing every index.html request — and
    // the sandbox force-installs this package into arbitrary user projects regardless
    // of the declared peer range. Vite 4+ prefers order/handler.
    transformIndexHtml: {
      order: "pre",
