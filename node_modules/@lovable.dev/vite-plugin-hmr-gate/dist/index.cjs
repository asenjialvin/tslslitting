...
function hmrGatePlugin(options = {}) {
  ...
  function fileContentHash(filePath) {
    try {
      return (0, import_node_crypto.createHash)("sha1").update((0, import_node_fs.readFileSync)(filePath)).digest("hex");
    } catch {
      return null;
    }
  }
      ...
      srv.middlewares.use((req, res, next) => {
        ...
        if (entries.length > 0) {
          for (const [f, events] of entries) {
            ...
            if (hash === null)
              lastFlushedHashes.delete(f);
            else
              lastFlushedHashes.set(f, hash);
          }
          if (fullReload) {
